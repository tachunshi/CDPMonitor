...
//        // 获取其他数据源配置
        Map slConfig = binder.bind("spring.datasource.slave1", Map.class).get();
        String sltypeStr = evn.getProperty("spring.datasource.master.type");
        Class<? extends DataSource> sltype = getDataSourceType(sltypeStr);
        defauleDataSourceProperties = slConfig;
        DataSource slbinsource = bind(sltype, defauleDataSourceProperties);
        customDataSources.put("slave1", slbinsource);
        ...
        logger.debug("注册数据源{}成功", "slave1");
        logger.debug("dataSourceRouterKey:"+dataSourceRouterKey);
//        List<Map> configs = binder.bind("spring.datasource.slave1", Bindable.listOf(Map.class)).get();
//        // 遍历从数据源
...
//        }
        ...
        customDataSources.keySet().stream().forEach(x->System.out.println(x));
